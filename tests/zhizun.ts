...
const token_json_file = "tests/zhizun_token.json";
...
async function getProduct(params: QUERY_BODY = data) {
  ...
  try {
    ...
    if (json_str && json_str.code === 0) {
      ...
      fs.writeFileSync(
        `json/zhizun/${params.premium ? "大圈" : "中圈"}zhizun_page_${
          params.page_index
        }.json`,
...
async function getDistrict() {
  ...
  mkdirIfNotExists("json/zhizun");
  fs.writeFileSync(
    `json/zhizun/zhizun_district.json`,
    JSON.stringify(json_str, null, 2)
  );
  return json_str;
}
...
async function saveAllDetails() {
  ...
  fs.writeFileSync(
    `json/all/zhizun_all_girls_details.json`,
    JSON.stringify(all_girls_details, null, 2)
  );
}
